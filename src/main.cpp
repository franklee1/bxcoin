...
unsigned int nStakeMaxAge = 96 * 60 * 60; // 96 hours
...
const string strMessageMagic = "bbx Signed Message:\n";
...
uint64_t LimitOrphanTxSize(uint64_t nMaxOrphans)
{
...
bool IsStandardTx(const CTransaction& tx)
{
    ...
    uint64_t sz = tx.GetSerializeSize(SER_NETWORK, CTransaction::CURRENT_VERSION);
                        ...
                        bool* pfMissingInputs)
{
...

    {
        ...
        uint64_t nSize = ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION);
...
uint64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{

            uint64_t nSubsidy = 1 * COIN;
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
            ...
            {
            nSubsidy = nSubsidy * 5 / 10 ;  //500% pos
            }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "bbx", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "bbxghdgdhhd";
        CTransaction txNew;
        txNew.nTime = 1525855736;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1525855736;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1325218 : 1325218;
        ...
        assert(block.hashMerkleRoot == uint256("0xfa3c162dbc5ae01c14ff76a40136750bfc06ca2045e34ae704c7ad9b8c971c11"));
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // bbx: send latest proof-of-work block to allow the
